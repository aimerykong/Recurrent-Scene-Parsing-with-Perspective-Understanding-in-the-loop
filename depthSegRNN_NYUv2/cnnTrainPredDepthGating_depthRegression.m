function [net,stats] = cnnTrainPredDepthGating_depthRegression(net, prefixStr, imdb, getBatch, varargin)
...
% net.layers(180).block.bnorm_moment_type_trn = 'global';
% net.layers(183).block.bnorm_moment_type_trn = 'global';
% idx=210, net.layers(idx), net.layers(idx).block
...
%         stats.val(epoch) = depthSegRNN_processEpoch_debug(net, state, opts.scalingFactor, opts, 'val', opts.numEpochs) ;
        [stats.train(epoch),prof] = depthSegRNN_processEpoch_depthRegression(net, state, opts.scalingFactor, opts, 'train', opts.numEpochs) ;
        stats.val(epoch) = depthSegRNN_processEpoch_depthRegression(net, state, opts.scalingFactor, opts, 'val', opts.numEpochs) ;
        ...
        spmd
            ...
            [stats_.train, prof_] = depthSegRNN_processEpoch_depthRegression(net_, state, opts.scalingFactor, opts, 'train') ;
            stats_.val = depthSegRNN_processEpoch_depthRegression(net_, state, opts.scalingFactor, opts, 'val') ;
    ...
    if opts.plotStatistics
        ...
        for p = plots
            ...
            elseif numel(plots) <= 12
                subplot(4, 3, find(strcmp(p,plots))) ;
            end
            ...
            title(sprintf('%s tsErr%.3f (%d) trErr%.3f (%d) ', p, min(values(2,:)), minIdx, min(values(1,:)),minIdxTr), 'Interpreter', 'none') ;
